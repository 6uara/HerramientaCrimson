...
func _process(delta: float) -> void:
	...
	# Sanidad: si la acción actual tiene un actor muerto, cancelarla
	if _current_action != null and not _current_action["actor"].get("is_alive"):
		print("[CM] Actor de _current_action murió, cancelando acción")
		active_player_char = null
		_finish_current_action()

	# Sanidad: limpiar la cola de actores muertos
	var filtered = []
	for a in _action_queue:
		if a["actor"].get("is_alive"):
			filtered.append(a)
...
func _tick_atb(delta: float) -> void:
	# Tick ATB jugadores. El ATB del jugador en cola/actuando NO avanza
	# (consistente con cómo manejamos los enemigos en cola).
	for p in player_chars:
		if not p.get("is_alive"):
			continue
		if _is_actor_in_queue_or_acting(p):
			continue
...
func _try_start_next_action() -> void:
	if _current_action != null or _action_queue.is_empty():
		return
	print("[CM] _try_start_next_action: queue size=", _action_queue.size())
...
func _start_action(action: Dictionary) -> void:
	print("[CM] _start_action: kind=", action.get("kind"), " actor=", action.get("actor"))
